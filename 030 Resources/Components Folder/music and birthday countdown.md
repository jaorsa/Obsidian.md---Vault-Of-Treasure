...
for (var ita=0;ita<files.length;ita++){
	...
	if (typeof(meta) != "undefined"){
		...
		if (birthday){
			...
			let hanBirthday = birthdayMonth + " month " + birthdayDay + " day ";
			...
			if (N < 31){
				count = count+1;
				final += `${basename} birthday is at ${hanBirthday}, ${N+1} day(s) to go \n`
			}
		}
	}
}

const element = <p>In the next month {count} birthdays:</p>;
...
const div2 = 
	  <div style={{whiteSpace:"pre-wrap"}}>
	  	<div>Nothing is working yet. </div>
		</div>
	  

const div3 = 
	  	 <div style={{whiteSpace:"pre-wrap"}}>
	  	<div>There is nothing to do right now. </div>
		</div>
