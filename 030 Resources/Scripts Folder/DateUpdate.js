...
// **  MUST EDIT TO YOUR OWN FORMAT (REPLACE "YYYY年MM月DD日#heading" TO YOUR OWN DAILY NOTE FORMAT)  **
    const todayDiaryFormat = "![[" + year + "-" + month + "-" + day + "%DailyQuest]]";
    const todayAgendaFormat = "![[" + year + "-" + month + "-" + day + "%TodaySchedule]]";
    // const yesterdayParrotFormat = "![[" + year + "-" + month + "-" + yesterday + "日#鸟崽]]";
// 读写文件
// READ AND EDIT FILE
    ...
    // /\!\[\[(\d{4})-(\d{2})-(\d{2})\#Diary\]\]/g
    // ")
    var result = data.replace(/\!\[\[(\d{4})-(\d{2})-(\d{2})\%DailyQuest\]\]/g,todayDiaryFormat);
    var result = result.replace(/\!\[\[(\d{4})-(\d{2})-(\d{2})\%TodaySchedule\]\]/g,todayAgendaFormat);
    // var result = result.replace(/\!\[\[(\d{4})-(\d{2})-(\d{2})日\#鸟崽\]\]/g,yesterdayParrotFormat);
