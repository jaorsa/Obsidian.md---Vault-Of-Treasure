...
status: in progress
target: 10000
banner: 030 Resources/Gallery Folder/pixel 57.gif
---
%% Profile Picture %%
...
<div class="second-image" style="width: 30%; height: 30%;" >
<svg xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:svgjs="http://svgjs.dev/svgjs" viewBox="0 0 800 800"><defs><linearGradient x1="50%" y1="0%" x2="50%" y2="100%" id="nnneon-grad"><stop stop-color="hsl(340, 45%, 50%)" stop-opacity="1" offset="0%"></stop><stop stop-color="hsl(340, 45%, 80%)" stop-opacity="1" offset="100%"></stop></linearGradient><filter id="nnneon-filter" x="-100%" y="-100%" width="400%" height="400%" filterUnits="objectBoundingBox" primitiveUnits="userSpaceOnUse" color-interpolation-filters="sRGB">
...
# Rainbell
- **AGENDA**
	- `$= '[['+moment().format("MMM Do, YYYY")+'|Notes of the day]]'`
	- `$= '[['+moment().format("YYYY, MMMM")+'|Monthly Note]]'`
	- `$= '[['+moment().format("YYYY🗓qQ ")+'|Quarterly Notes]]'`
	-  `$= '[['+moment().format("YYYY📆")+'|Annual Notes]]'`
	- [[00. Goals for the past five years | Five-year plan]]
- **LIFE**
	- [[00. Book List|Library]]
	- [[Weight Problem|Weight]]
	- [Calendar](obsidian://advanced-uri?vault=%E5%85%AB%E5%96%9C&filepath=00.%2520Homepage.md&commandid=big-calendar%253Aopen-big-calendar)
...
%% Music iframe and birthday countdown %%
<br>
```jsx:
<Show></Show>
```
...
%% Display diary, schedule, bird record in separate columns %%

%%

````ad-col2
```dataviewjs
dv.paragraph(`![[${moment().format("YYYY-MM-DD") + "diary"}]]`)
```

```dataviewjs
dv.paragraph(`![[${moment().format("YYYY-MM-DD") + "schedule"}]]`)
```
...
<s class="aside-hide"> Writing to see how much lines I can write before I need to add another space between lines, really wonder how it's going to turn out </s>
````ad-col2

## Daily Quest
---
![[2022-03-26%DailyQuest]]

![[retro 28.webp#center|200]]

## Today's Schedule
---
![[2022-03-26%TodaySchedule]]
...
\* Track project documents marked with the #project  tag.
%%
...
**CURRENTLY READING** ![[No More Mr. Nice Guy.jpg#right|180]]
...
<p align = "center">No More Mr. Nice Guy <br>
Dr. Robert A. Glover</p>
...
```dataviewjs
dv.table(["Book Title", "Author", "Type", "Status"], dv.pages("#reading")
...
let totalDays = "You have used *Obsidian* for "+total+" days "
let nofold = '!"misc/templates"'
let allFile = dv.pages(nofold).file
let totalMd = "Co-created "+
	allFile.length+" Article | ==Notes== "
let totalTag = allFile.etags.distinct().length+" 个==tags=="
let totalTask = allFile.tasks.length+" 个==To-do==。 "
dv.paragraph(
...
```dataviewjs
for (let group of dv.pages("#type").groupBy(p => p.type)) {
    dv.header(3, group.key);
    dv.table(["Name", "Status", "Due Date"],
        group.rows
            .sort(k => k.difficulty, 'desc')
            .map(k => [k.file.link, k.status, k["Due Date"]]))
}
```
```ActivityHistory
/
